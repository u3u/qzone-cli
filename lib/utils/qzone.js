...
module.exports = class Qzone {
  ...
  async downloadPhotosByAlbumId(albumid, dest) {
    process.execSync(`mkdir -p ${dest}`)
    ...
    const downloadAll = async (ps = 0, pn = 20) => {
      ...
      if (ps <= 0) spinner.succeed('Fetch photos successful.')
      return Promise.all(
        photos.map(url =>
          download(url).then(data => {
            fs.writeFileSync(path.join(dest, `${md5(data)}.png`), data)
            bar.tick()
          })
        )
